...
public class SessionManagement3 extends HttpServlet {
	...
	/**
	 * Users must use this functionality to sign in as an administrator to retrieve
	 * the result key. If the user name is valid but not the passwor, an error
	 * message with the user name is returned.
	 * 
	 * @param userName
	 *            Sub schema user name
	 * @param password
	 *            Sub schema user password
	 */
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (Validate.validateSession(ses)) {
			...
			try {
				...
				callstmt = conn
						.prepareStatement("SELECT userName, userAddress, userRole FROM users WHERE userName = ?");
				...
				if (resultSet.next()) {
					...
					if (resultSet.getString(3).equalsIgnoreCase("admin")) {
						...
						callstmt = conn.prepareStatement(
								"SELECT userName, userAddress, userRole FROM users WHERE userName = ? AND userPassword = SHA(?)");
						callstmt.setString(1, subName);
						...
						if (resultSet2.next()) {
							...
							String userKey = Hash.generateUserSolution(levelResult,
									(String) ses.getAttribute("userName"));

							htmlOutput = "<h2 class='title'>" + bundle.getString("response.welcome") + " "
									+ Encode.forHtml(resultSet2.getString(1)) + "</h2>" + "<p>"
									+ bundle.getString("response.resultKey") + " <a>" + userKey + "</a>" + "</p>";
						} else {
							userAddress = bundle.getString("response.badPass") + " <a>"
									+ Encode.forHtml(resultSet.getString(1)) + "</a><br/>";
							htmlOutput = makeTable(userAddress, bundle);
						}
					} else {
						log.debug("Successful Guest Login");
						htmlOutput = makeTable(bundle) + "<h2 class='title'>"
								+ bundle.getString("response.welcomeGuest") + "</h2>" + "<p>"
								+ bundle.getString("response.guestMessage") + "</p><br/><br/>";
					}
				} else {
	...
	private static String makeTable(String userAddress, ResourceBundle bundle) {
		return "<table>" + userAddress + "<tr><td>" + bundle.getString("form.userName")
				+ "</td><td><input type='text' id='subUserName'/></td></tr>" + "<tr><td>"
				+ bundle.getString("form.password") + "</td><td><input type='password' id='subUserPassword'/></td></tr>"
				+ "<tr><td colspan='2'><div id='submitButton'><input type='submit' value='"
				+ bundle.getString("form.signIn") + "'/>" + "</div></td></tr>" + "</table>";
	}

	private static String makeTable(ResourceBundle bundle) {
		return "<table><tr><td>" + bundle.getString("form.userName")
				+ "</td><td><input type='text' id='subUserName'/></td></tr>" + "<tr><td>"
				+ bundle.getString("form.password") + "</td><td><input type='password' id='subUserPassword'/></td></tr>"
				+ "<tr><td colspan='2'><div id='submitButton'><input type='submit' value='"
				+ bundle.getString("form.signIn") + "'/>" + "</div></td></tr>" + "</table>";
