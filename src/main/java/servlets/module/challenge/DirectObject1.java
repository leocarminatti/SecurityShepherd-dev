...
/**
 * Insecure Direct Object Challenge Challenge One Does not use user specific key
 * because key is currently hard coded into database schema <br/>
 * <br/>
...
public class DirectObject1 extends HttpServlet {
	...
	// OWASP Sanitizer
	PolicyFactory policy = new HtmlPolicyBuilder().allowElements("a", "img").allowUrlProtocols("https")
			.allowAttributes("href", "src").onElements("a", "img").requireRelNofollowOnLinks().toFactory();

	/**
	 * The user must abuse this functionality to reveal a hidden user. The result
	 * key is hidden in this users profile.
	 * 
	 * @param userId
	 *            To be used in generating the HTML output
	 */
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ResourceBundle bundle = ResourceBundle.getBundle("i18n.servlets.challenges.directObject.directObjectRef1",
				locale);
		...
		if (Validate.validateSession(ses)) {
			...
			try {
				...
				if (resultSet.next()) {
					...
					htmlOutput = "<h2 class='title'>" + userName + "'s " + bundle.getString("response.message")
							+ "</h2>" + "<p>" + privateMessage + "</p>";
				} else {
					log.debug("No Profile Found");

					htmlOutput = "<h2 class='title'>" + bundle.getString("response.notFound") + "</h2><p>"
							+ bundle.getString("response.notFoundMessage.1") + " '" + Encode.forHtml(userId) + "' "
