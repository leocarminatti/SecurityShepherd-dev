...
public class SessionManagement2 extends HttpServlet {
	...
	/**
	 * The user attempts to use this function to sign into a sub schema. If they
	 * successfully sign in then they are able to retrieve the result key for the
	 * challenge If they sign in with a correct user name but incorrect password
	 * then the email address of the user will be returned in a error message
	 * 
	 * @param subName
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ResourceBundle bundle = ResourceBundle
				.getBundle("i18n.servlets.challenges.sessionManagement.sessionManagement2", locale);
			...
			try {
				...
				if (resultSet.next()) {
					...
					htmlOutput = "<h2 class='title'>" + bundle.getString("response.welcome") + " "
							+ Encode.forHtml(resultSet.getString(1)) + "</h2>" + "<p>"
	...
	private static String makeTable(String userAddress, ResourceBundle bundle) {
		return "<table>" + userAddress + "<tr><td>" + bundle.getString("form.userName")
				+ "</td><td><input type='text' id='subName'/></td></tr>" + "<tr><td>"
				+ bundle.getString("form.password") + "</td><td><input type='password' id='subPassword'/></td></tr>"
