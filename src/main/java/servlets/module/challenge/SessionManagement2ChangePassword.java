...
/**
 * Session Management Challenge Two - Password Reset Servlet Does not return
 * result key <br/>
 * <br/>
...
public class SessionManagement2ChangePassword extends HttpServlet {
	...
	/**
	 * A user with the submitted email address is set a new random password, the
	 * password is also returned from the database procedure and is forwards through
	 * to the HTTP response. This response is not consumed by the client interface
	 * by default, and the user will have to discover it.
	 * 
	 * @param subEmail
	 *            Sub schema user email address
	 */
			...
			try {
				...
				try {
					PreparedStatement callstmt = conn
							.prepareStatement("UPDATE users SET userPassword = SHA(?) WHERE userAddress = ?");
