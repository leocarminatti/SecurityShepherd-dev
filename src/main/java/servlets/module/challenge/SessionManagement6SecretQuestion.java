...
/**
 * Session Management Challenge Six - Security Question Does not return result
 * key <br/>
 * <br/>
...
public class SessionManagement6SecretQuestion extends HttpServlet {
	...
	/**
	 * A user submits a username and answer, these values are checked against the DB
	 * to see if they are valid
	 * 
	 * @param subEmail
	 *            Sub schema user email to search DB with
	 * @param subAnswer
	 *            Sub schema user secret answer to check against the DB
	 */
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ResourceBundle bundle = ResourceBundle
				.getBundle("i18n.servlets.challenges.sessionManagement.sessionManagement6", locale);
					...
					if (Validate.isValidEmailAddress(subEmail) && subAns.length() > 5) {
						...
						if (rs.next()) {
							...
							htmlOutput = "<h2 class='title'>" + bundle.getString("response.welcome") + " "
									+ Encode.forHtml(rs.getString(1)) + "</h2>" + "<p>"
									+ bundle.getString("response.welcome") + " <a>" + userKey + "</a>" + "</p>";
						} else {
							log.debug("Bad Answer Submitted");
							htmlOutput = new String("<h2 class='title'>" + bundle.getString("question.badAnswer")
									+ "</h2><p>" + bundle.getString("question.whoAreYou"));
						}
					} else {
	...
	/**
	 * A user submits an email address to get that user's Secret QUestion. This is
					...
					if (decodedCookie.equals("doNotReturnAnswers")) // Untampered Cookie
					{
						...
						try {
							if (subEmail.length() < 10) {
								...
								// PreparedStatement callstmt = conn.prepareStatement("SELECT secretQuestion
								// FROM users WHERE userAddress = \"" + subEmail +"\"");
								// ResultSet rs = callstmt.executeQuery();

								if (rs.next()) {
									log.debug("'Valid' User Detected");
									log.debug("Encoding for output: " + rs.getString(1));
									// rs.getString(1) contains the question for the user to answer. This question
									// is asked in English as it must be answered in English to successfully pass
									// the level
